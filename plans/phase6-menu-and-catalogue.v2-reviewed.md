...
- Q1.3 The CCR routability RPC must not block the first draw. `uwpick.mjs` draws from the snapshot plus a cached routable set, then issues the RPC with a 400 ms timeout and redraws the dim state when it returns (Tasks A5 and A10). A timeout or a refusal is not an error: the cached set from `~/.uw/state/routable.json` is used, and rows whose routability is unknown render undimmed.
...
- Q2.1 Catalogue snapshot missing, unreadable, or not JSON: the picker prints one line naming the file and the `uw catalog refresh` command that rebuilds it, then exits 1 so the chat input is left untouched (Task A8).
...
- Q2.3 `~/.uw/state/picker.json` missing or corrupt: recents and favourites start empty, the picker runs normally, and the corrupt file is renamed to `picker.json.corrupt-<stamp>` rather than deleted (Task A7).
...
- Q2.5 CCR RPC unreachable, slow, or returning an unexpected shape: see Q1.3. The picker never blocks on it and never fails because of it (Task A4).
- Q2.6 `CONIN$` cannot be opened: the wrapper prints `uwpick: cannot open CONIN$` with the Win32 message and exits 1, and Claude Code discards the buffer (Task A12).
...
- Q2.8 Every write to a state or catalogue file is atomic: write `<file>.tmp-<pid>`, flush, then rename over the target. A crash mid-write leaves the previous file intact. This covers `picker.json`, `routable.json`, `startup.json`, `health.json`, `capabilities.json`, `install.json`, `snapshot.json` and the catalogue `current` pointer (Tasks A7, A8 and B5).

**Q3 — User friendly.**

- Q3.1 The help line is always visible, on the last line of the frame, at both levels and in flat scope (Tasks A9 and A10).
- Q3.2 A filter that matches nothing renders an explicit empty-state row rather than a blank area: `no match for "<query>" — backspace to widen, esc to clear` (Tasks A6 and A9).
...
- Q4.3 No other file in the repository may contain the strings `claude-code-router`, `node_modules`, `.claude`, `APPDATA` or a loopback address. Task A4 adds a test that greps the tree and fails on a violation, which is what keeps the boundary real rather than documented.
...
- Q6.3 `menu/hud-shim.mjs` (Task A14) is an optional statusline wrapper, installed only with `install.ps1 -Hud`, under 60 lines, that corrects that one field and passes everything else through untouched. Any failure inside the shim forwards the original stdin verbatim, so the footer degrades to today's behaviour rather than breaking.
...
- Q7.2 Animation is event-driven only. The input loop is a blocking `readSync` on `//./CONIN$`, so there is no event loop while a key is awaited and timer-driven idle animation is impossible without a worker thread. No worker thread is added.
...
**Scenario 4 — the statusline shim eats the footer.** The optional HUD shim (Task A14) is installed, then something downstream changes: the OMC HUD path moves, the wrapped command is edited by hand, the snapshot is missing, or Claude Code changes the statusline stdin shape. Symptom: the footer goes blank or shows an error string on every prompt, which reads as an OMC failure and will be debugged in the wrong repository. Mitigation: the shim is optional and off by default; every failure path inside it forwards the original stdin bytes to the wrapped command unmodified, so the worst outcome is today's behaviour; the install records the previous `statusLine.command` verbatim so uninstall restores it byte for byte; and `uw doctor` asserts both that the wrapped command still exists and that the shim round-trips a sample payload with the expected fields intact.
...
| `C:/Users/osami/.claude/settings.json` | Optional and only with `install.ps1 -Hud`: `statusLine.command` is rewritten to invoke `hud-shim.mjs` in front of the existing command, which is stored verbatim for a byte-for-byte restore |
...
| `C:/Users/osami/.uw/state/routable.json` | Last-known routable model set from CCR, used as the fallback when the RPC is slow or absent |
...
`CC.CONTRACT` — frozen `{product, fingerprint, handoff:{argvIndex, acceptExit, discardExit}, command:{model}, statusline:{modelPath, contextPath, usageKeys}}`.
...
`CCR.readService() -> {origin, token} | null`.
...
test("no file outside the two contract modules names Claude Code or CCR", () => {
  ...
  for (const dir of ["menu", "refresh"]) {
    ...
    for (const f of fs.readdirSync(d)) {
      if (allowed.has(f) || !/\.mjs$/.test(f)) continue;
...
export const CONTRACT = Object.freeze({
  ...
  fingerprint: "service.json + /api/ccr/rpc + x-ccr-web-auth",
  servicePath: path.join(APPDATA, "claude-code-router", "service.json"),
  rpcPath: "/api/ccr/rpc",
  authHeader: "x-ccr-web-auth",
  tokenParam: "ccr_web_token",
  bundledCatalogue: "C:/nvm4w/nodejs/node_modules/@musistudio/claude-code-router/dist/models.json",
});
...
Expected: `# pass 27`, `# fail 0`. The boundary test passes trivially at this point because `menu/` holds only `sanitize.mjs` and the two contract modules; it earns its keep from Task A5 onward, and Task B4 extends its scan to `keysync/` once that directory's hard-coded catalogue path is gone.
...
`readJsonOr(file: string, fallback: any) -> any` — parse or return the fallback; never throws.
`cachedRoutable() -> {set: Set<string>, at: string | null}`
...
`routableSet(opts?: {timeoutMs?: number, rpc?: Function, write?: boolean}) -> Promise<{set: Set<string>, fresh: boolean}>` — 400 ms budget, cache-backed, never throws
...
import fs from "node:fs";
import path from "node:path";
import { priceOf, isTextOut, badgeOf, buildFrom, routableSet } from "../menu/catalog.mjs";
...
test("routableSet falls back to the cache instead of dimming everything", async () => {
  ...
  const dead = await routableSet({ rpc: async () => null, write: false });
  assert.equal(dead.fresh, false);
  assert.equal(dead.set instanceof Set, true);   // the cache, empty or not -- never a throw
});
```
 ...
 *
 * Three changes from the spike version, all of them Q1.3 and Q4.1:
 *   - the RPC itself now lives in ccr-client.mjs, so this file names no CCR path;
 *   - the timeout drops from 8000 ms to 400 ms, because this call is on the path
 *     to a visible frame and 8 seconds of blank terminal is a worse outcome than
 *     a few rows that are not dimmed;
 *   - a successful answer is cached to ~/.uw/state/routable.json, and a failed one
 *     falls back to that cache rather than to an empty set. An empty set meant
 *     "dim everything", which is the most alarming possible rendering of "the
 *     gateway did not answer in time".
 */
export const ROUTABLE_CACHE = path.join(os.homedir(), ".uw", "state", "routable.json");

export function cachedRoutable() {
  try {
    const j = JSON.parse(fs.readFileSync(ROUTABLE_CACHE, "utf8"));
    return { set: new Set(j.ids ?? []), at: j.at ?? null };
  } catch { return { set: new Set(), at: null }; }
}

export async function routableSet({ timeoutMs = 400, rpc = CCR.rpc, write = true } = {}) {
  const cfg = await rpc("getConfig", [], { timeoutMs });
  if (!cfg) return { set: cachedRoutable().set, fresh: false };
  const set = CCR.routableFromConfig(cfg);
  if (write) {
    try {
      writeAtomic(ROUTABLE_CACHE,
        JSON.stringify({ at: new Date().toISOString(), ids: [...set] }, null, 2));
    } catch { /* a cache we cannot write is still a set we can use */ }
  }
...
`reduce(state: State, ev: string | {resize: number}) -> {state: State, exit: null | {target: string|null}, favourite: null | string}`
...
**The design.** Level 0, illustrative, at the standard 78-column frame:

```text
╭─ UW ▸ providers ─────────────────────────────────────────────────────────────╮
│  filter: gem▏                          44 providers · 1584 models · 08-24    │
│                                                                              │
...
```text
│  no match for "zzz" — backspace to widen, esc to clear                       │
```
...
`frame(v: View, meta: Meta, opts: {caps, routable: Set<string>}) -> string[]`
...
import { detectCaps, motionEnabled, glyphsFor, painter, badgeColour, proportionBar,
         ...
         slideFrames, flashFrames, revealFrames, FRAME_W } from "../menu/style.mjs";
...
const ROWS = [
  { keyId: "personal.google.free", provider: "google", free: 3, planCount: 0, health: "ok",
...
test("the proportion bar reads at a glance and never lies about missing data", () => {
  ...
  assert.equal(proportionBar(null, 12, g), "     "); // no price data: no bar at all
  assert.equal(proportionBar(0, 0, g), "     ");
...
test("the health dot carries the state in the glyph as well as the colour", () => {
  ...
  assert.notEqual(healthDot("ok", g, p), healthDot("broken", g, p));
  assert.equal(healthDot("ok", glyphsFor(PLAIN), painter(PLAIN)), "*");
});
...
test("glyphs fall back to ASCII with no escape sequences at all", () => {
  ...
  const lines = frame(V0, META, { caps: PLAIN, routable: new Set() });
...
test("level 0 renders the exact provider row, columns included", () => {
  ...
  assert.equal(row,
    "|> personal.google.free" + " ".repeat(11) + "      3   ###..  3" +
    " ".repeat(11) + "* ok" + " ".repeat(7) + " |");
});

test("level 1 renders the exact model row, columns included", () => {
  const lines = frame(V1, META, { caps: PLAIN, routable: new Set(["google/gemini-3.5-flash-lite"]) });
  const row = lines.find((l) => l.includes("gemini-3.5-flash-lite"));
  assert.equal(row,
    "|> gemini-3.5-flash-lite" + " ".repeat(13) + "    1M    0.00    0.00  FREE  " +
    "TVR" + " ".repeat(4) + " |");
});
...
test("every line of every frame is exactly FRAME_W visible columns", () => {
  for (const v of [V0, V1, { ...V0, empty: true, items: [], filter: "zzz" },
    ...
    for (const caps of [VT, PLAIN]) {
      for (const l of frame(v, META, { caps, routable: new Set() })) {
        assert.equal(strip(l).length, FRAME_W, `width ${strip(l).length}: ${strip(l)}`);
      }
    }
  }
});
...
test("the empty state names the query, and the legend replaces the rows", () => {
  const e = frame({ ...V0, empty: true, items: [], filter: "zzz" }, META,
                  { caps: PLAIN, routable: new Set() }).join("\n");
  assert.match(e, /no match for "zzz"/);
  assert.equal(e.includes("personal.google.free"), false);
  const l = frame({ ...V0, legend: true }, META, { caps: PLAIN, routable: new Set() }).join("\n");
  assert.match(l, /\[\^f\]/);
  assert.match(l, /\[\?\]/);
});
...
test("the help line is present on every frame including the empty one", () => {
  for (const v of [V0, V1, { ...V0, empty: true, items: [], filter: "zzz" }]) {
    const lines = frame(v, META, { caps: PLAIN, routable: new Set() });
...
test("transitions are a fixed, bounded number of frames", () => {
  const base = frame(V1, META, { caps: PLAIN, routable: new Set() });
...
const UNI = {
  marker: "▶", fav: "★", recent: "↺", dotOk: "●", dotStale: "○",
  on: "▰", off: "▱", check: "✔", arrow: "→", sep: "▸", caret: "▏", ell: "…", dash: "—",
  frame: { tl: "╭", tr: "╮", bl: "╰", br: "╯", h: "─", v: "│" },
};
const ASCII = {
  marker: ">", fav: "*", recent: "~", dotOk: "*", dotStale: "o",
...
export function proportionBar(free, total, g, width = 5) {
...
export function healthDot(health, g, p) {
  if (health === "broken") return p.red(g.dotOk);
  if (health === "needs $") return p.yel(g.dotOk);
...
const rpad = (s, n) => sanitizeDisplay(String(s ?? ""), n).padStart(n);
const ctxS = (c) => (c == null ? "" : c >= 1e6 ? `${c / 1e6}M` : `${Math.round(c / 1000)}k`);
const money = (v) => (v == null ? "" : Number(v).toFixed(2));
const strip = (s) => s.replace(/\x1b\[[0-9;]*m/g, "");

// Pad to the frame's inner width using the VISIBLE length, so a coloured cell
// does not shorten its row. This is the one place colour and layout interact.
const bar = (g, body) => {
  const visible = strip(body).length;
  return `${g.frame.v}${body}${" ".repeat(Math.max(0, INNER - visible))} ${g.frame.v}`;
};

const title = (g, p, text) => {
  const head = `${g.frame.tl}${g.frame.h} ${text} `;
  return p.ramp(head) + g.frame.h.repeat(Math.max(0, FRAME_W - strip(head).length - 1)) + g.frame.tr;
};
...
const footer = (g, p, text) => {
  ...
  return p.dim(head) + g.frame.h.repeat(Math.max(0, FRAME_W - strip(head).length - 1)) + g.frame.br;
};
...
export function frame(v, meta, { caps, routable }) {
  ...
  const right = v.level === 0
    ? `${meta.providers} providers ${g.sep} ${meta.models} models ${g.sep} ${String(meta.generatedAt).slice(5, 10)}`
    : `${v.items.length} of ${v.provider.models.length}`;
  const left = `  filter: ${sanitizeDisplay(v.filter, 40)}${p.inv(g.caret)}`;
  const gap = Math.max(1, INNER - strip(left).length - right.length);
  ...
  if (v.empty) {
    L.push(bar(g, `  no match for "${sanitizeDisplay(v.filter, 30)}" ${g.dash} backspace to widen, esc to clear`));
  }
  ...
  v.items.forEach((it, i) => {
    ...
    if (it.kind === "provider") {
      ...
      body = `${mark} ` + highlight(pad(r.keyId, W.keyId), v.filter, p) +
             ...
             healthDot(r.health, g, p) + " " + pad(r.health, W.health - 2);
      ...
      // Not routable right now is information, not an error: dim the row, keep it.
      if (!routable.has(it.target) && routable.size) body = p.dim(strip(body));
    }
...
Expected: `# pass 107`, `# fail 0`. If the two exact-row tests fail, read the diff before touching them: they encode the column widths constraints 6 and 7 pin, and the usual cause of a mismatch is a padding change that also silently moved every other column.
...
The ordering inside `main()` is the whole point of Q1.3. The snapshot read and the first frame happen first, synchronously; only then is the routability RPC issued, with a 400 ms budget, and the frame redrawn with the dim state applied. A picker that awaited the gateway before drawing would show a blank terminal for as long as the gateway felt like taking, which is the failure mode most likely to make the feature feel broken when nothing is broken.
...
**Interfaces:** Consumes: `loadSnapshot` from `./snapshot.mjs`; `routableSet` from `./catalog.mjs`; `initState`, `reduce`, `view` from `./pick-state.mjs`; `frame`, `confirmLine`, `detectCaps`, `glyphsFor`, `painter`, `motionEnabled`, `slideFrames`, `revealFrames`, `flashFrames`, `sleepSync`, `FRAME_MS` from `./style.mjs`; `handoffTarget`, `modelCommand`, `CONTRACT` from `./cc-contract.mjs`; `loadPickerState`, `recordRecent`, `toggleFavourite`, `recordHandoff`, `recordStartup` from `./state.mjs`. Produces:
`screen(v: View, meta: Meta, opts: {caps, routable}) -> string` — the frame as one string, exported so it is testable without a console
...
const show = (v, routable = ROUTABLE, caps = CAPS) => plain(screen(v, META, { caps, routable }));
...
test("the model header names the six required columns", () => {
  ...
  assert.match(show(view(s)), /model\s+ctx\s+\$in\s+\$out\s+badge\s+caps/);
});
...
test("a non-routable model row is dimmed rather than hidden", () => {
  const s = reduce(initState(ROWS), "\r").state;
  const raw = screen(view(s), META, { caps: VT, routable: ROUTABLE });
...
test("escape sequences in a model id cannot reach the frame", () => {
  ...
  const s = reduce(initState(rows), "\r").state;
  assert.equal(screen(view(s), META, { caps: VT, routable: new Set() }).includes("\x1b[2J"), false);
});
...
import { routableSet } from "./catalog.mjs";
...
export function firstFrame({ snap, recents, favourites, caps, termRows }) {
  ...
  // The empty routable set means "we do not know yet", and style.mjs treats an
  // empty set as "dim nothing" precisely so the first frame is never a wall of
  // grey that then lights up (Q1.3).
  return { state, meta, text: screen(view(state), meta, { caps, routable: new Set() }) };
}
...
export async function main() {
  ...
  if (!loaded.ok) {
    // Q2.1: exit non-zero so Claude Code discards the buffer and the chat input
    // the user already typed survives untouched.
    process.stderr.write(failMessage({ ...loaded, detail: loaded.detail ?? SNAPSHOT_FILE }) + "\n");
  ...
  let routable = new Set();

  const lines = () => frame(view(state), meta, { caps, routable });
  ...
  // Only now, with a frame already on screen, do we ask the gateway anything.
  ...
  const finish = (target) => {
    let wrote = false;
    try {
      if (FILE && target != null) { fs.writeFileSync(FILE, modelCommand(...target.split(/\/(.*)/s))); wrote = true; }
    ...
    out.write(CLEAR + SHOW + (target ? confirmLine(target, g, p) + "\n" : ""));
  ...
  for (;;) {
    ...
    if (r.exit) {
      closeSync(CONIN);
      if (r.exit.target) {
        recordRecent(r.exit.target);
        run("select", view(state).cursor - view(state).top + 4);
      }
      finish(r.exit.target);
    }
...
Three deliberate changes from the spike beyond the wiring. The key log is gone: it existed to prove keystrokes arrived at all, that is now established, and an append-only log of every keystroke typed into a filter box is a small liability for no remaining benefit. `finish()` records the handoff shape instead, which is what `uw doctor` reads. And the selection is turned into a command by `modelCommand`, so the one string Claude Code parses exists in exactly one file.
...
// The child prints one number and nothing else, so a stray console.log anywhere
// in the import graph turns into a parse failure rather than a silent wrong
// measurement.
const CHILD = `
const t0 = process.hrtime.bigint();
const { loadSnapshot } = await import("file://${MENU}/snapshot.mjs");
const { firstFrame } = await import("file://${MENU}/uwpick.mjs");
const { detectCaps } = await import("file://${MENU}/style.mjs");
...
export async function sampleOnce() {
  const { stdout } = await pexec(process.execPath, ["--input-type=module", "-e", CHILD],
                                 { timeout: 30000 });
  const ms = Number(stdout.trim());
  if (!Number.isFinite(ms)) throw new Error(`bench child printed ${JSON.stringify(stdout)}`);
...
Expected: `# pass 125`, `# fail 0`, and the standalone run prints five samples and `PASS first frame <n> ms (budget 300 ms)`. If it fails, the diagnosis order is: snapshot size first (`~/.uw/catalog/snapshot.json` should be well under 2 MB), then whether anything in the import graph reached `keysync.mjs` — which pulls in a PowerShell round trip and would show up as a multi-second sample rather than a marginal one.

Then measure the wrapper's share once, for the record rather than for the gate:

```
powershell -NoProfile -Command "$t=Get-Date; & 'C:/Users/osami/.uw/menu/uwpick-run.ps1' -File $null -ChildScript 'noop.mjs'; (Get-Date)-$t"
```

Record the observed total in `~/.uw/docs/visual-design.md` under a short "measured costs" heading, so the next person to wonder why ctrl+g is not instantaneous has the breakdown rather than a suspicion.
...
Expected: `# pass 129`, `# fail 0`. If `CreateFile` on `CONIN$` fails because the test host has no console, the first test fails with `ENOENT ... conmode.json` and the wrapper's own message `uwpick: cannot open CONIN$`; run the suite from a real Windows Terminal rather than a piped harness.
...
**Interfaces:** Consumes: `parseStatusline`, `usedTokens` from `./cc-contract.mjs`; `loadSnapshot`, `contextIndex` from `./snapshot.mjs`. Produces:
...
`main(argv?: string[]) -> number` — read stdin, transform, spawn the wrapped command with the result on its stdin, return its exit code
...
import { loadSnapshot, contextIndex } from "./snapshot.mjs";
...
export function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    if (snap.ok) payload = transform(raw, contextIndex(snap.snap)) ?? raw;
  } catch { payload = raw; }
...
Expected: `# pass 153`, `# fail 0`. The shim is not installed by this task; Task A16 wires it, and only when asked.
...
export function diagnose({ env, handoff, current, pinned, contracts, hud }) {
...
export function main() {
  ...
  const r = diagnose({
    ...
    contracts: { ccObserved: current.ccVersion, ccPinned: CC.CONTRACT.fingerprint,
...
Expected: `# pass 169`, `# fail 0`. The `doctor` run prints three lines; on a machine where the installer has not run yet it prints `RED editor-wiring EDITOR is "(unset)" ...` and exits 1, which is correct until Task A16.
...
# ---- optional: the statusline shim (Q6.3) ---------------------------------
# Off unless asked. It edits ONE key in Claude Code's settings.json and stores the
# previous value verbatim, so -Hud -Uninstall is a byte-for-byte restore rather
# than a reconstruction. It never touches an OMC file.
if ($Hud) {
  ...
  if ($HudUninstall) {
    ...
    Write-Host "hud: restored -> $($prevHud.previousCommand)"
    exit 0
  }
  ...
  @{ previousCommand = $currentCommand; wrappedAt = (Get-Date).ToUniversalTime().ToString("o")
     settings = $settingsPath } | ConvertTo-Json | Set-Content -Path $hudState -Encoding UTF8
  $json.statusLine.command = $wrapped
  ($json | ConvertTo-Json -Depth 20) | Set-Content -Path $settingsPath -Encoding UTF8
...
Two details in that block are load-bearing. `ConvertTo-Json -Depth 20` is required because PowerShell's default depth of 2 silently flattens nested settings objects into type names, which would corrupt `settings.json` on write. And the copy to `settings.json.uw-bak` exists because the round trip through `ConvertFrom-Json` and `ConvertTo-Json` reformats the whole file even where it changes nothing; the backup is the byte-for-byte original, and `-Hud -Uninstall` restores the command value rather than the formatting.
...
Expected: `# pass 178`, `# fail 0`. The `-WhatIf` run prints the two planned assignments and changes nothing. Run it for real without `-WhatIf` before Task A17.
...
Expected: `# pass 191`, `# fail 0`. The denylist's `console.warn` will appear in the output for the two tests that exercise rejection; that is the intended behaviour, not test noise.
...
Expected: `# pass 199`, `# fail 0` once Task B4 has run; before B4, the last test in this file fails with `ENOENT ... catalog\models.json` and everything else passes. The `--dry` run must still print the same provider count as before the change; the *selection* will differ, which is expected and is what Task B9 applies deliberately.
...
Expected: `# pass 211`, `# fail 0`. Confirm by eye that `~/.uw/menu/uwpick.mjs` now shows `FREE` (no question mark) on OpenRouter's `:free` models and blank on huggingface's zero-priced rows.
...
export function resolveCatalogPath() {
  try {
    ...
    const p = path.join(VERSIONS, stamp, "models.json");
...
Expected: `# pass 218`, `# fail 0`, and the `--dry` run prints `catalog: 217 providers, generated 2026-08-24T12:22:28.162Z` as before — the source moved, the content did not.
...
export function writeSnapshot(snapshot) {
  ...
  for (const d of fs.readdirSync(VERSIONS).sort().slice(0, -3)) {
...
Expected: `# pass 239`, `# fail 0`. The dry run prints `models.dev: HTTP 200, ~212 providers`, one line per provider, and `--dry-run: nothing written.`
...
Expected: `# pass 254`, `# fail 0`, and the dry run prints `providers: 44 (floor 44)`.
...
Expected: `# pass 259`, `# fail 0`. Diff the dry run against the committed `built-rows.json` and expect the selection to differ — that is the point, and it is why this is a deliberate run rather than a drive-by edit. Then, with no Claude Code session mid-request:
...
test("no rendered cell contains a control character", () => {
  ...
  const opts = { caps: detectCaps({ WT_SESSION: "1" }, 120), routable: new Set() };
  let s = initState(rows);
...
function rebuildPickerSnapshot() {
  const file = writeSnapshotFile(buildSnapshot(build()));
  console.log(`picker snapshot rebuilt: ${file}`);
  return file;
}
```

Call `rebuildPickerSnapshot()` from `main()` immediately after the line that writes the `current` pointer, and skip it under `--dry-run` along with every other write.
...
Expected: `# pass 268`, `# fail 0`; the tier 1 refresh writes a catalogue version, flips `current`, prints `picker snapshot rebuilt: ...`, and `doctor` reports green. Then re-run the Task A17 protocol steps P2 and P6 and confirm the badges and the free column reflect the curated cadence.
